...
public class IndexController {
    ...
    private void addTourListToModel(String numOfRows, String pageNo, String area, Model model) throws IOException {
        try {
            model.addAttribute("tourList", indexService.extractTours(indexService.getDataTour(numOfRows, pageNo, area)));
    ...
    private void addTourListToModel(String numOfRows, String pageNo, String area, String arrange, String mapX, String mapY, Model model) throws IOException {
        try {
            if (arrange.equals("S")) {
                model.addAttribute("tourList", indexService.extractTours2(indexService.getDataTour2(numOfRows, pageNo, arrange, mapX, mapY)));
            } else {
                model.addAttribute("tourList", indexService.extractTours(indexService.getDataTour(numOfRows, pageNo, area)));
            }
        } catch (JSONException e) {
    ...
    private void addEventListToModel(String numOfRows, String pageNo, String year, String month, Model model) throws IOException {
        try {
            model.addAttribute("eventList", indexService.extractEvents(indexService.getDataEvent(numOfRows, pageNo, year, month)));
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    private void addTourInfoToModel(String contentId, String contentTypeId, Model model) throws IOException {
        try {
            model.addAttribute("tourInfo", indexService.extractTourInfo(indexService.getTourInfo(contentId, contentTypeId)));
            model.addAttribute("tourInfo2", indexService.extractTourInfo2(indexService.getTourInfo3(contentId, contentTypeId)));
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    private void addEventInfoToModel(String contentId, String contentTypeId, Model model) throws IOException {
        try {
            model.addAttribute("eventInfo", indexService.extractEventInfo(indexService.getEventInfo(contentId, contentTypeId)));
            model.addAttribute("eventInfo2", indexService.extractEventInfo2(indexService.getEventInfo3(contentId, contentTypeId)));
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
