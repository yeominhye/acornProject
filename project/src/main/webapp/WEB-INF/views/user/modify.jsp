...
$(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data == "duplicate") {
                        ...
                        $("input:not(#nickname), button").on('mousedown click keydown focusin', function(e) {
                            e.preventDefault();
                            return false;
                        });
                    } else {
                        ...
                        $("#submitBtn").prop("disabled", false);
                        $('input:not(#nickname), button').off('mousedown click keydown focusin');
                    }
                }
            });
        }
    ...
    $("#userBirth").on('input', function() {
        var userBirth = $(this).val().replace(/[^0-9]/g, '');
        var formattedBirth = formatBirth(userBirth);
        $(this).val(formattedBirth);
        
        if (userBirth.length !== 8 && userBirth !== '') {
        		$("#userBirthCheckMsg").text("올바른 날짜를 입력해주세요.");
            	$("#submitBtn").prop("disabled", true);
            	$("input:not(#userBirth), button").on('mousedown click keydown focusin', function(e) {
                    e.preventDefault();
                    return false;
                });
        } else {
    ...
    $("#userPw").on('input', function() {
        ...
        if (!regex.test(userPw)) {
            ...
            $("#submitBtn").prop("disabled", true);
            $("input:not(#userPw), button").on('mousedown click keydown focusin', function(e) {
                e.preventDefault();
                return false;
            });
        } else {
            $("#pwError").text("");
            $("#submitBtn").prop("disabled", false);
            $('input:not(#userPw), button').off('mousedown click keydown focusin');
        }
    });
    ...
    function formatBirth(userBirth) {
        var formatted = userBirth.replace(/(\d{4})(\d{2})(\d{2})/, '$1-$2-$3');
        return formatted;
    }
...
<div class="container">
    <div class="form-container">
        ...
        <form name="myForm" action="${pageContext.request.contextPath}/user/modify_process.do" method="post">
            <table class="input-table">
                <tr class="form-table-tr1">
                    <td class="form-label">아이디</td>
                    <td class="form-input"><input type="text" name="userId" class="input-box" value="${user.userId}" readonly><br>*아이디는 변경하실 수 없습니다.</td>
                </tr>
                <tr class="form-table-tr1">
                    <td class="form-label">비밀번호</td>
                    <td class="form-input"><input type="password" class="input-box" name="userPw" id="userPw" value="${user.userPw}" required><br><span id="pwError" style="color:red;"></span><br>* 영문자, 숫자 조합하여 만들어야 합니다.</td>
                </tr>
                ...
                <tr class="form-table-tr2">
                    ...
                    <td class="form-input"><input type="text" name="userName" class="input-box" value="${user.userName}" required></td>
                </tr>
                <tr class="form-table-tr1">
                    <td class="form-label">생년월일</td>
                    <td class="form-input"><input type="text" id="userBirth" name="userBirth" class="input-box" value='<fmt:formatDate value="${user.userBirth}" pattern="yyyy-MM-dd"/>'/><br><span id="userBirthCheckMsg" style="color:red;"></span><br>* 생년월일은 8자리로 입력해야 합니다.  ex) 1990-02-13</td>
                </tr>
                <tr class="form-table-tr2">
                    <td class="form-label">전화번호</td>
                    <td class="form-input"><input type="text" name="userTel" id="userTel" class="input-box" value="${user.userTel}" required><br><span id="userTelCheckMsg" style="color:red;"></span></td>
                </tr>
            </table>
